...
const DropzoneComponent = ({ id, maxFiles, setFiles, files }) => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    try {
      ...
      if (response?.isError) {
        ...
        const updatedFiles = files?.concat(response?.images);
  ...
  const thumbs = useMemo(() => files?.map((file, index) => (
      <>
        <div
  ...
  return (
    <div className="container">
      <div
        style={{ display: loading ? "none" : "flex", cursor: "pointer" }}
        id={`${id}-dropzone`}
        {...getRootProps({
          className: "dropzone",
